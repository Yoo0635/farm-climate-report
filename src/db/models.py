...
class Brief(Base):
    ...
    actions: Mapped[list["BriefAction"]] = relationship(
        back_populates="brief", cascade="all, delete-orphan"
    )
    signals: Mapped[list["BriefSignal"]] = relationship(
        back_populates="brief", cascade="all, delete-orphan"
    )
    draft_reports: Mapped[list["DraftReport"]] = relationship(
        back_populates="brief", cascade="all, delete-orphan"
    )
...
class DraftReport(Base):
    ...
    refined_report: Mapped[Optional["RefinedReport"]] = relationship(
...
class RefinedReport(Base):
    ...
    draft_id: Mapped[str] = mapped_column(
        ForeignKey("draft_reports.id", ondelete="CASCADE"), unique=True
    )
    content: Mapped[str] = mapped_column(Text)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=datetime.utcnow
