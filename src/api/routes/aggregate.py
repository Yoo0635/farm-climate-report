...
from src.services.aggregation import AggregateEvidencePack, AggregateRequest, get_aggregation_service
...
@router.post("/api/aggregate", response_model=AggregateEvidencePack, status_code=status.HTTP_200_OK)
async def aggregate_endpoint(payload: AggregateRequest, demo: bool | None = Query(None)) -> AggregateEvidencePack:
    """
    ...
    try:
        ...
        _log_failure(effective_payload, req_id, started, demo, "not_implemented", str(exc))
        raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail=str(exc)) from exc
    except RuntimeError as exc:
        _log_failure(effective_payload, req_id, started, demo, "upstream_unavailable", str(exc))
