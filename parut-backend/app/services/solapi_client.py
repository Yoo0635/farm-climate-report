...
def _auth_header() -> dict:
    ...
    sig  = hmac.new(_API_SECRET.encode(), (date + salt).encode(), hashlib.sha256).hexdigest()
    return {"Authorization": f"HMAC-SHA256 apiKey={_API_KEY}, date={date}, salt={salt}, signature={sig}"}
...
def _mk_rcs_message(to: str, text: str, quick: List[Tuple[str, str]]) -> dict:
    ...
    return {
        ...
        "type": typ,
        "rcsOptions": {
            "quickReplies": [{"label": lbl, "payload": payload} for lbl, payload in quick],
            "disableSms": True
        }
    }
