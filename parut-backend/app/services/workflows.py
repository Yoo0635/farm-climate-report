...
DUMMY_JSONL = REPO_ROOT / "data" / "dummy_llm_results.jsonl"
...
def _compose_user_prompt(action: str, ctx: Optional[Dict[str,Any]], free_text: str|None=None) -> str:
    base = {
    ...
    if action=="DONE":
        instruct = "작업 완료 보고다. 완료 확인과 후속 관리 2~3가지만 핵심 bullet로 제시해라."
    elif action=="HELP":
...
def run_numeric_workflow(msisdn: str, code: str, free_text: str|None=None) -> Dict[str,Any]:
