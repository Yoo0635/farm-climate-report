...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--insect-key",
        default=os.environ.get("NPMS_DEFAULT_INSECT_KEY"),
        help="SVC53 insectKey (상세조회키).",
    )
    parser.add_argument(
        "--sido-code",
        default=os.environ.get("NPMS_DEFAULT_SIDO"),
        help="시도코드 (예: 경북=47).",
    )
    parser.add_argument(
        "--service-type",
        default=os.environ.get("NPMS_SVC53_TYPE", "AA003"),
        choices=["AA001", "AA003"],
    ...
    parser.add_argument(
        "--show-curl",
        action="store_true",
        help="Print a sanitized curl command before request.",
    )
    parser.add_argument(
        "--sigungu", help="Filter results to rows whose sigunguNm contains this text."
    )
    parser.add_argument(
        "--raw", action="store_true", help="Print raw response body (truncated)."
    )
