...
def send_one(to: str, text: str) -> Dict[str, Any]:
    if not svc or not SOLAPI_FROM:
        return {"to": to, "group_id": None, "failed": [{"status_code": "DRYRUN", "status_message": "환경변수 없음(드라이런)"}]}
    if to == SOLAPI_FROM:
        return {"to": to, "group_id": None, "failed": [{"status_code":"SELF","status_message":"FROM과 TO가 동일"}]}
    try:
        ...
        if fm:
            return {"to": to, "group_id": None, "failed": [{"status_code":getattr(f,"status_code",None),"status_message":getattr(f,"status_message",None)} for f in fm]}
...
def main():
    ...
    ap.add_argument("--file", default=str((here.parents[1] / "data" / "llm_results.jsonl").resolve()))
    ...
    with src.open("r", encoding="utf-8") as f, logf.open("a", encoding="utf-8") as lf:
        for i, line in enumerate(f, 1):
            ...
            recips = to_list(args.to) if args.to else to_list(",".join(rec.get("recipients") or []))
            if not recips:
                lf.write(json.dumps({"src_line": i, "from": SOLAPI_FROM, "result": {"error": "NO_RECIPIENT"}}) + "\n")
                continue
            ...
            for to in recips:
                if args.max and count >= args.max:
                    break
                if args.dry_run:
                    res = {"to": to, "group_id": None, "failed": [{"status_code":"DRYRUN","status_message":"사용자 요청(드라이런)"}]}
                else:
                    res = send_one(to, body)
                lf.write(json.dumps({"src_line": i, "from": SOLAPI_FROM, "result": res}, ensure_ascii=False) + "\n")
                print(json.dumps(res, ensure_ascii=False))
                count += 1
            if args.max and count >= args.max:
                break

    print(json.dumps({"ok": True, "from": SOLAPI_FROM, "sent": count, "log": str(logf)}, ensure_ascii=False))
